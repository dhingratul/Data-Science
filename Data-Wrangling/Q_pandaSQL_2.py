...
# -*- coding: utf-8 -*-
"""
Created on Fri Jun 23 13:28:37 2017
...
def aggregate_query(filename):
    ...
    # lowercase, so the
    # column names more closely resemble columns names one might find in table.

    aadhaar_data = pandas.read_csv(filename)
    aadhaar_data.rename(columns=lambda x: x.replace(' ', '_').lower(),
                        inplace=True)

    # Write a query that will select from the aadhaar_data table how many men
    # and how
    # many women over the age of 50 have had aadhaar generated for them in
    # each district.
    # aadhaar_generated is a column in the Aadhaar Data that denotes the number
    # who have had
    # aadhaar generated in each row of the table.
    #
    # Note that in this quiz, the SQL query keywords are case sensitive.
    # For example, if you want to do a sum make sure you type 'sum' rather than
    # 'SUM'.
    #
